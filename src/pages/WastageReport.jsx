...
export default function WastageReport() {
  ...
  // Listen to plan, compute missed status, and save to Firebase immediately
  useEffect(() => {
    if (!user) return;
    ...
    onValue(r, snap => {
      ...
      // Build the per-session object for a given date
      const buildObj = (allMissed) => {
        ...
        withStatus.forEach(s => {
          obj[s.id] = {
            ...
            missed:      allMissed !== undefined ? allMissed : s.missed,
          };
        });
        return obj;
      };

      // Save today immediately on every plan change
      const todayDate = dateStr(new Date());
      const todayObj  = buildObj(undefined);
      console.log("[WastageReport] saving today:", { uid: user.uid, date: todayDate, data: todayObj });
      saveWastage(user.uid, todayDate, todayObj)
        .then(() => console.log("[WastageReport] save SUCCESS for", todayDate))
        .catch(err => console.error("[WastageReport] save FAILED:", err));

      // Auto-record yesterday once per login session (if entry missing)
  ...
  return (
    <div>
      <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 16 }}>Today's Wastage</h2>
      ...
      {/* Today's table */}
      <div style={{ background: "white", borderRadius: 10, overflow: "hidden", border: "1px solid #ddd9d2", marginBottom: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {todayWastage.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: "center", padding: 40, color: "#6b6560" }}>
                ...
                return (
                  <tr key={s.id} onClick={() => setSelectedId(isSel ? null : s.id)}
                    style={{ borderBottom: "1px solid #ddd9d2", cursor: "pointer", background: isSel ? "#eaf4ef" : "white" }}>
                    <td style={{ padding: "12px 14px", fontWeight: 500 }}>
                      {s.name}
                      {s.subject && <div style={{ fontSize: 11, color: "#6b6560", marginTop: 2 }}>{s.subject}</div>}
                    </td>
                    <td style={{ padding: "12px 14px", fontFamily: "monospace" }}>{fmt12(s.start)}</td>
                    <td style={{ padding: "12px 14px", color: "#e63946", fontWeight: 600 }}>MISSED</td>
                    <td style={{ padding: "12px 14px", fontFamily: "monospace", color: "#e63946" }}>{toHHMMSS(dur(s.start, s.end))}</td>
                    <td style={{ padding: "12px 14px" }}>
                      <span style={{ background: "#fde8e8", color: "#e63946", padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>YES</span>
                    </td>
                  </tr>
                );
              })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginBottom: 8 }}>
        <SummaryCard label="Missed Today"  value={todayWastage.length}     color="#e63946" />
...
function SummaryCard({ label, value, color }) {
  return (
    <div style={{ background: "white", borderRadius: 10, padding: "20px 16px", textAlign: "center", border: "1px solid #ddd9d2", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
...
function outlineRed(disabled) {
  return { background: disabled ? "#f8f6f2" : "white", color: disabled ? "#aaa" : "#e63946", border: `1.5px solid ${disabled ? "#ddd9d2" : "#e63946"}`, borderRadius: 8, padding: "9px 20px", fontSize: 13, fontWeight: 500, cursor: disabled ? "not-allowed" : "pointer" };
}
