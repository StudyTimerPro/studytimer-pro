...
export default function TodaysPlan() {
  ...
  async function handleSave() {
    ...
    if (editingId) {
      await updatePlan(user.uid, editingId, data);
      showToast("Session updated ✓");
    } else {
      await savePlan(user.uid, data);
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        ...
        <button onClick={openAdd} style={btnStyle("#2d6a4f")}>＋ Add Session</button>
      </div>

      {/* Table */}
      <div style={{ background: "white", borderRadius: 10, overflow: "hidden", border: "1px solid #ddd9d2", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead style={{ background: "#1a1814", color: "white" }}>
              <tr>
            ...
            <tbody>
              {sessions.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: "center", padding: 48, color: "#6b6560" }}>
                  <div style={{ fontSize: 40 }}>📋</div>
                  <p style={{ marginTop: 12 }}>No sessions yet. Add your first study session!</p>
                </td></tr>
              ) : sessions.map(s => (
                <tr key={s.id} style={{ borderBottom: "1px solid #ddd9d2" }}>
                  <td style={{ padding: "12px 14px" }}>
                    <div style={{ fontWeight: 500 }}>{s.name}</div>
                    {s.subject && <div style={{ fontSize: 11, color: "#6b6560", marginTop: 2 }}>{s.subject}</div>}
                  </td>
                  <td style={{ padding: "12px 14px" }}><Badge p={s.priority} /></td>
                  <td style={{ padding: "12px 14px", fontFamily: "monospace", fontSize: 13 }}>{fmt12(s.start)}</td>
                  <td style={{ padding: "12px 14px", fontFamily: "monospace", fontSize: 13 }}>{fmt12(s.end)}</td>
                  <td style={{ padding: "12px 14px" }}>
                    <span style={{ fontFamily: "monospace", fontSize: 12, background: "#f0f0f0", borderRadius: 6, padding: "3px 8px" }}>
                      {minsToHM(duration(s.start, s.end))}
                    </span>
                  </td>
                  <td style={{ padding: "12px 14px", fontSize: 12, fontFamily: "monospace" }}>
                    {s.breakMins > 0 ? `${fmt12(s.end)} – ${fmt12(addMins(s.end, s.breakMins))}` : <span style={{ color: "#aaa" }}>No break</span>}
                  </td>
                  <td style={{ padding: "12px 14px" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      <button onClick={() => startSession(s)}    style={iconBtn("#2d6a4f", "white")}>▶</button>
      ...
      {modalOpen && (
        <div onClick={e => e.target === e.currentTarget && setModalOpen(false)}
          ...
          <div style={{ background: "white", borderRadius: 14, padding: 28, width: "min(480px, 94vw)", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <h3 style={{ fontSize: 18, marginBottom: 20 }}>{editingId ? "Edit Session" : "Add Study Session"}</h3>

            <Field label="Session Name">
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Calculus: limits & continuity" style={inputStyle} />
            </Field>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              <button onClick={() => setModalOpen(false)} style={{ background: "none", border: "1.5px solid #ddd9d2", borderRadius: 8, padding: "9px 20px", fontSize: 14, cursor: "pointer" }}>Cancel</button>
              <button onClick={handleSave} style={btnStyle("#2d6a4f")}>Save Session</button>
            </div>
          </div>
        </div>
      )}
...
function Chip({ label, value }) {
  return (
    <div style={{ background: "white", border: "1px solid #ddd9d2", borderRadius: 10, padding: "8px 16px", fontSize: 13, fontFamily: "monospace" }}>
      {label}: <span style={{ fontWeight: 700, color: "#2d6a4f" }}>{value}</span>
    </div>
  );
}
...
function Field({ label, children }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
      <label style={{ fontSize: 12, fontWeight: 600, color: "#6b6560", textTransform: "uppercase", letterSpacing: 0.4 }}>{label}</label>
      {children}
    </div>
  );
}

const inputStyle = { width: "100%", padding: "9px 12px", border: "1.5px solid #ddd9d2", borderRadius: 8, fontSize: 14, background: "#f0ede8", color: "#1a1814", fontFamily: "inherit" };
